...
impl <Ok, Err, Store: ParseStore<Pos, V> + ?Sized, Pos: ParsePos, V: ParseValue, Child: ParseNode<Ok, Err, Store, Pos, V>, const N: usize> ParseNode<[Ok; N], Err, Store, Pos, V> for [Child; N] {
    fn do_parse<'a>(&self, cxt: ParseContext<'a, Store, Pos, V>) -> ParseResult<[Ok; N], Err, Pos> {
        let mut accume: [MaybeUninit<Ok>; N] = unsafe { MaybeUninit::<[MaybeUninit<Ok>; N]>::uninit().assume_init() };
        let mut curr_pos = cxt.pos.clone();

        for len in 0..N {
            match self[len].do_parse(cxt.with_pos(curr_pos.clone())) {
                Okay(ok, advance) => {
    ...
    fn do_parse_span<'a>(&self, cxt: ParseContext<'a, Store, Pos, V>) -> ParseResult<Span<Pos>, Err, Pos> {
        ...
        // parse each child
        for child in self {
            match child.do_parse_span(cxt.with_pos(curr_pos.clone())) {
